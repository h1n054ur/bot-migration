...
def prompt_choice(prompt_text: str, choices: list[str], default: str = "0") -> str:
    ...
    """
    return Prompt.ask(prompt_text, choices=choices, default=default, show_choices=True)

def print_panel(text: str, title: str | None = None, style: str = 'info', content: str = ""):
    ...
    Args:
        ...
        content (str, optional): Additional content to display below the panel
    """
    panel = Panel.fit(text, title=title, style=STYLES[style])
    console.clear() if content else None
    console.print(panel)
    if content:
...
def print_table(headers: list[str], rows: list[list[str]]):
    ...
    Args:
        headers (list[str]): List of column titles
